...
export const useMonitorDeviceTelemetry = (deviceId: number | null, params?: { from?: string; to?: string; limit?: number; offset?: number }) => useQuery({ queryKey: [...KEYS.all, "telemetry", deviceId, params], queryFn: () => monitorService.getDeviceTelemetry(deviceId!, params), enabled: !!deviceId, refetchInterval: 15000 });
export const useMonitorDeviceTelemetryHistory = (deviceId: number | null, params?: { from?: string; to?: string; limit?: number; offset?: number }) => useQuery({ queryKey: [...KEYS.all, "telemetry-history", deviceId, params], queryFn: () => monitorService.getDeviceTelemetry(deviceId!, params), enabled: !!deviceId });
